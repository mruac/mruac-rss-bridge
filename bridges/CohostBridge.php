...
class CohostBridge extends BridgeAbstract
{
	...
	public function collectData()
	{
		...
		*/
		...
		$data = $data ? array_reduce($data['queries'], function ($acc, $v) {if (str_contains($v['queryHash'], '["posts","profilePosts"]')) {return $acc[] = $v;}}, []) : null;
	...
	private function parsePost(array $post)
	{
					...
					case 'attachment':
						...
						switch ($block['kind']) {
							...
							case 'audio':
								...
								$attach_str .= "<figure>{$alt_text}<audio controls src=\"{$block['fileURL']}\"><a href=\"{$block['fileURL']}\"> Download audio: {$title} </a></audio></figure>";
								$enclosures[] = $block['fileURL'];
								break;
						}
						break;
		...
		if(strlen($post_tags)> 0 ){$post_tags = "<p>🏷 Tag(s): {$post_tags}</p>";}
